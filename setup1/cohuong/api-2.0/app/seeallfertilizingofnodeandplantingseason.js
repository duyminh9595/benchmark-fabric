...
const query = async (channelName, chaincodeName, email, plantingseason) => {

    try {
        const ccp = await helper.getCCP("cohuong") //JSON.parse(ccpJSON);

        // Create a new file system based wallet for managing identities.
        const walletPath = await helper.getWalletPath("cohuong") //.join(process.cwd(), 'wallet');
        ...
        result = await contract.submitTransaction(
            'getFertilizingbymspandidplantingseason', plantingseason
        )
